...
const fs = require("fs"),
  ...
  // dayjs = require("dayjs"),
  getWeek = require("./utils/getWeek.js");
  
const Year = new Date();
const ThisWeekInYear = getWeek() - 1;
const TYPE_ARR = ["JavaScript", "TypeScript", "CSS", "HTML", "Vue"];
// const TIME = dayjs().format("YYYY-MM-DD");
const URL = (type) => `https://github.com/trending/${type}?since=weekly`;
const MarkDownFileTitle = (type) =>
  `# [GitHub] ${type} 周趋势榜项目(第${ThisWeekInYear}周)\n\n`;

const FilePath = (type) => path.resolve(__dirname, `../DATA/weekly-${type}`);
...
(async () => {
  ...
  for (let type_i = 0; type_i < TYPE_ARR.length; type_i++) {
    ...
    // 文件写入
    fs.writeFile(
      FILEPATH + `/${Year.getFullYear()}-${ThisWeekInYear}-${Year}.md`,
      MarkDownFile,
      (err) => {
        if (!err) {
